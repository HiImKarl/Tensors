...
public:
    ...
    Tensor(const Tensor<T>& tensor):
        dimensions_(tensor.dimensions_),
        is_owner_(true) {
        elements_ = pArrayCopy(tensor.elements_, 
        pVectorProduct(dimensions_));
    }
...
template <typename T>
template <typename... Args>
Tensor<T> Tensor<T>::pAccessExpansion(vector<uint16_t>& indices, uint16_t next_index, Args... rest) {
    if (next_index >= dimensions_.at(indices.size())) {
        std::cout << "Index " << indices.size() << " (" << next_index << ") Out of Bounds\n";
        return Tensor<T>();
    }
...
std::ostream & operator<<(std::ostream& os, const Tensor<T>& tensor) {
    uint32_t indices_product = Tensor<T>::pVectorProduct(tensor.dimensions_);
    os << tensor.elements_[0] << ((tensor.dimensions_.size() == 0) ? "" : " | ");
    for (uint32_t i = 1; i < indices_product - 1; ++i) {
        os << tensor.elements_[i] << ' ';
        uint32_t total_elems = Tensor<T>::pVectorProduct(tensor.dimensions_);
        for (uint16_t j = 0; j < tensor.dimensions_.size(); ++j) {
            total_elems /= tensor.dimensions_.at(j);
            if ((i + 1) % total_elems == 0) {
                os << Tensor<T>::pRepeatString("|", tensor.dimensions_.size() - j) << ' ';
                break;
            }
            os << ' ';
        }
...
template <typename T>
void Tensor<T>::operator+=(const Tensor<T>& tensor) {
    if (dimensions_ != tensor.dimensions_) {
...
Tensor<T> operator+(const Tensor<T>& tensor_1, const Tensor<T>& tensor_2) {
    if (tensor_1.dimensions_ != tensor_2.dimensions_) {
        std::cout << "operator+(Tensor, Tensor) Failed :: Incompatible Dimensions\n";
        return Tensor<T>();
    }
